...
public interface IDShortAllocMapper {
    ...
    @Update("UPDATE leaf_alloc SET max_id = ((max_id + step) & ~(-1 << bit_size)) WHERE biz_tag = #{tag}")
    void updateMaxId(@Param("tag") String tag);

    @Update("UPDATE leaf_alloc SET max_id = ((max_id + #{step}) & ~(-1 << bit_size)) WHERE biz_tag = #{key}")
